...
# Modification to the original code to import the models from the MODELS_DIR
import os
...
class pyramid_trans_expr2(nn.Module):
    def __init__(self, img_size=224, num_classes=8, window_size=[28,14,7], num_heads=[2, 4, 8], dims=[64, 128, 256], embed_dim=768):
        ...
        # Modification to the original code to import the models from the MODELS_DIR
        face_landback_checkpoint = torch.load(os.path.join(MODELS_DIR, "Poster_V2", "mobilefacenet_model_best.pth.tar"),
        ...
        # Modification to the original code to import the models from the MODELS_DIR
        ir_checkpoint = torch.load(os.path.join(MODELS_DIR, "Poster_V2", "ir50.pth"), map_location=lambda storage, loc: storage)
